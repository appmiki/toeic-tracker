...
const PART_LIST = [
  { id: "p1", label: "Part1" }, { id: "p2", label: "Part2" }, { id: "p3", label: "Part3" },
  { id: "p4", label: "Part4" }, { id: "p5", label: "Part5" }, { id: "p6", label: "Part6" },
  { id: "p7", label: "Part7" },
];
...
async function loadPartScores() {
  try {
    const raw = localStorage.getItem("toeic:part-scores");
    return raw ? JSON.parse(raw) : [];
  } catch { return []; }
}
async function savePartScores(partScores) {
  try { localStorage.setItem("toeic:part-scores", JSON.stringify(partScores)); } catch {}
}
...
export default function App() {
  ...
  const [partScores, setPartScores] = useState(null);
  const [monthIdx, setMonthIdx] = useState(1); // default: 8月
  const [scoreDraft, setScoreDraft] = useState({ date: todayStr(), score: "" });
  const [partDraft, setPartDraft] = useState({ date: todayStr(), part: "p5", correct: "", total: "" });
  ...
  useEffect(() => {
    (async () => {
      ...
      setPartScores(await loadPartScores());
    })();
  }, []);
  ...
  const handleDeleteScore = useCallback(async (dateStr) => {
    const next = (scores || []).filter(s => s.date !== dateStr);
    setScores(next);
    await saveScores(next);
  }, [scores]);

  const handleAddPartScore = useCallback(async () => {
    const correct = Number(partDraft.correct);
    const total = Number(partDraft.total);
    if (!total || correct < 0 || correct > total) return;
    const entry = { id: `${partDraft.date}-${partDraft.part}-${Date.now()}`, date: partDraft.date, part: partDraft.part, correct, total };
    const next = [...(partScores || []), entry].sort((a, b) => a.date.localeCompare(b.date));
    setPartScores(next);
    await savePartScores(next);
    setPartDraft(d => ({ ...d, correct: "", total: "" }));
  }, [partDraft, partScores]);

  const handleDeletePartScore = useCallback(async (id) => {
    const next = (partScores || []).filter(s => s.id !== id);
    setPartScores(next);
    await savePartScores(next);
  }, [partScores]);
  ...
  const handleExport = useCallback(() => {
    const payload = {
      ...
      done: done || {}, scores: scores || [], partScores: partScores || [],
    };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `toeic-tracker-${todayStr()}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  }, [done, scores, partScores]);
  ...
  const handleImportFile = useCallback((file) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (typeof data.done !== "object" || !Array.isArray(data.scores)) throw new Error("invalid");
        setImportPreview(data);
      } catch {
        setImportError("ファイルの形式が正しくありません。書き出したJSONファイルを選んでください。");
      }
    };
  ...
  const handleConfirmImport = useCallback(async () => {
    ...
    const nextDone = importPreview.done || {};
    const nextScores = importPreview.scores || [];
    const nextPartScores = importPreview.partScores || [];
    setDone(nextDone); setScores(nextScores); setPartScores(nextPartScores);
    await saveDone(nextDone); await saveScores(nextScores); await savePartScores(nextPartScores);
    ...
    <div style={styles.app}>
      ...
      <div style={styles.hero}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
          <button onClick={handleExport} style={styles.dataBtn}>⤓ 書き出す</button>
          <label style={{ ...styles.dataBtn, textAlign: "center" }}>
            ⤒ 読み込む
            <input
        ...
        {importError && <div style={{ fontSize: 12, color: "#F27059", marginTop: 8 }}>{importError}</div>}
        {importPreview && (
          <div style={styles.importBox}>
            <div style={{ fontSize: 12.5, color: "#EAF0F7", marginBottom: 8 }}>
              このデータで今の記録を上書きします。よろしいですか?
            </div>
            <div style={{ fontSize: 11.5, color: "#8CA0BC", marginBottom: 10 }}>
              完了日数 {Object.keys(importPreview.done || {}).length}件 ・ 模試スコア {(importPreview.scores || []).length}件 ・ パート記録 {(importPreview.partScores || []).length}件
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={handleConfirmImport} style={{ ...styles.dataBtn, flex: 1, background: "linear-gradient(90deg,#4FD1AE,#3fc79e)", color: "#0F1A2E", border: "none" }}>上書きする</button>
              <button onClick={() => setImportPreview(null)} style={{ ...styles.dataBtn, flex: 1 }}>キャンセル</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={styles.content}>
        ...
        {tab === "progress" && (
          <ProgressTab chartData={chartData} scoreDraft={scoreDraft} setScoreDraft={setScoreDraft}
            onAddScore={handleAddScore} onDeleteScore={handleDeleteScore} scores={scores} done={done}
            partScores={partScores} partDraft={partDraft} setPartDraft={setPartDraft}
            onAddPartScore={handleAddPartScore} onDeletePartScore={handleDeletePartScore} />
        )}
...
function ProgressTab({
  chartData, scoreDraft, setScoreDraft, onAddScore, onDeleteScore, scores, done,
  partScores, partDraft, setPartDraft, onAddPartScore, onDeletePartScore,
}) {
  return (
    <div>
      <div style={styles.card}>
      ...
      {scores.length > 0 && (
        <div style={styles.card}>
          <div style={styles.cardTitle}>記録一覧</div>
          {scores.slice().reverse().map(s => (
            <div key={s.date} style={styles.scoreRow}>
              <span style={{ color: "#8CA0BC", fontSize: 13 }}>{s.date}</span>
              <span style={{ color: "#EAF0F7", fontWeight: 600, fontSize: 13 }}>{s.score}</span>
              <button onClick={() => onDeleteScore(s.date)} style={styles.deleteBtn} aria-label="削除">✕</button>
            </div>
          ))}
        </div>
      )}
      ...
      <PartScoreSection
        partScores={partScores} partDraft={partDraft} setPartDraft={setPartDraft}
        onAddPartScore={onAddPartScore} onDeletePartScore={onDeletePartScore}
      />
    </div>
  );
}
...
/* ---------------------------------------------------------
   パート別スコアの記録
--------------------------------------------------------- */
function PartScoreSection({ partScores, partDraft, setPartDraft, onAddPartScore, onDeletePartScore }) {
  const byPart = useMemo(() => {
    const map = {};
    PART_LIST.forEach(p => { map[p.id] = { correct: 0, total: 0 }; });
    (partScores || []).forEach(s => {
      if (!map[s.part]) return;
      map[s.part].correct += s.correct;
      map[s.part].total += s.total;
    });
    return map;
  }, [partScores]);
  ...
  return (
    <div>
      <div style={styles.card}>
        <div style={styles.cardTitle}>パート別スコアの記録</div>
        <input
          type="date" value={partDraft.date}
          onChange={e => setPartDraft(d => ({ ...d, date: e.target.value }))}
          style={styles.dateInput}
        />
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <select
            value={partDraft.part}
            onChange={e => setPartDraft(d => ({ ...d, part: e.target.value }))}
            style={{ ...styles.numInput, flex: 1 }}
          >
            {PART_LIST.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
          </select>
          <input
            type="number" min="0" placeholder="正解数" value={partDraft.correct}
            onChange={e => setPartDraft(d => ({ ...d, correct: e.target.value }))}
            style={{ ...styles.numInput, flex: 1 }}
          />
          <input
            type="number" min="1" placeholder="問題数" value={partDraft.total}
            onChange={e => setPartDraft(d => ({ ...d, total: e.target.value }))}
            style={{ ...styles.numInput, flex: 1 }}
          />
        </div>
        <button onClick={onAddPartScore} style={styles.saveBtn}>記録を追加</button>
      </div>

      <div style={styles.card}>
        <div style={styles.cardTitle}>パート別 正答率(累計)</div>
        {PART_LIST.map(p => {
          const { correct, total } = byPart[p.id];
          const pct = total ? Math.round((correct / total) * 100) : 0;
          return (
            <div key={p.id} style={{ marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, marginBottom: 4 }}>
                <span>{p.label}</span>
                <span style={{ color: "#8CA0BC" }}>{total ? `${correct}/${total} (${pct}%)` : "記録なし"}</span>
              </div>
              <div style={styles.scoreBarTrack}>
                <div style={{ ...styles.scoreBarFill, width: `${pct}%` }} />
              </div>
      ...
      {(partScores || []).length > 0 && (
        <div style={styles.card}>
          <div style={styles.cardTitle}>パート別 記録一覧</div>
          {partScores.slice().reverse().map(s => {
            const label = PART_LIST.find(p => p.id === s.part)?.label || s.part;
            const pct = Math.round((s.correct / s.total) * 100);
            return (
              <div key={s.id} style={styles.scoreRow}>
                <span style={{ color: "#8CA0BC", fontSize: 13 }}>{s.date}</span>
                <span style={{ color: "#F2B84B", fontSize: 12.5, width: 44 }}>{label}</span>
                <span style={{ color: "#EAF0F7", fontWeight: 600, fontSize: 13 }}>{s.correct}/{s.total}({pct}%)</span>
                <button onClick={() => onDeletePartScore(s.id)} style={styles.deleteBtn} aria-label="削除">✕</button>
              </div>
            );
          })}
        </div>
...
const styles = {
  ...
  scoreRow: { display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid #1E2C46" },
  deleteBtn: { marginLeft: "auto", width: 26, height: 26, borderRadius: 8, border: "1px solid #3A2A2A", background: "rgba(242,112,89,0.1)", color: "#F27059", fontSize: 12, cursor: "pointer", lineHeight: 1 },
  dataBtn: { flex: 1, padding: "9px 0", borderRadius: 10, border: "1px solid #26374F", background: "#0F1A2E", color: "#EAF0F7", fontSize: 12.5, fontWeight: 500, cursor: "pointer", fontFamily: "'Noto Sans JP',sans-serif" },
  importBox: { marginTop: 12, padding: 12, borderRadius: 10, border: "1px solid rgba(242,184,75,0.35)", background: "rgba(242,184,75,0.06)" },
